...
      </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="w-full max-w-[600px] flex items-center gap-4">
              <div className="relative flex-1">
                ...
                <input type="text" placeholder="Search London area..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-holo-white border-2 border-holo-teal rounded-[32px] focus:outline-none focus:ring-2 focus:ring-holo-coral placeholder:text-holo-coral" />
              </div>
              
              {/* Proceed Button */}
              <button onClick={handleProceed} disabled={!selectedBuilding} className={`px-6 py-3 rounded-xl font-inter font-semibold text-[16px] transition-all duration-300 shadow-md whitespace-nowrap ${selectedBuilding ? 'bg-gradient-teal-coral hover:bg-gradient-coral-teal text-holo-white hover:scale-105' : 'bg-gray-200 text-gray-500 cursor-not-allowed'}`}>
        ...
        <div className="absolute right-[calc(50vw-300px-12rem)] top-1/2 transform -translate-y-1/2 flex flex-col items-center space-y-6">
          ...
          <div className="text-center">
            ...
            <div className="w-full h-full flex items-center justify-center bg-holo-coral/20">
                ...
                </div> : selectedBuilding ? <div className="w-20 h-12 bg-holo-teal rounded"></div> : <span className="text-gray-400 text-sm font-inter text-center px-4">No Building Selected</span>}
            </div>
            {selectedBuilding && <p className="text-sm font-inter text-gray-600">
          ...
          <div className="flex flex-col gap-6">
            <button onClick={handleAnalysisClick} disabled={!selectedBuilding} className={`w-32 h-32 flex flex-col items-center justify-center rounded-full border-2 transition-all duration-300 ${!selectedBuilding ? 'bg-gray-100 border-gray-300 text-gray-400 cursor-not-allowed' : analysisClicked ? 'bg-holo-coral border-holo-coral text-holo-white' : 'bg-holo-teal/20 border-holo-teal/30 text-holo-teal/70 hover:bg-holo-coral hover:border-holo-coral hover:text-holo-white hover:shadow-lg'} hover:scale-105`}>
              <Building size={48} />
              <span className="text-sm font-inter mt-2">Analysis</span>
            </button>

            <button onClick={handleAdvancedClick} disabled={!selectedBuilding} className={`w-32 h-32 flex flex-col items-center justify-center rounded-full border-2 transition-all duration-300 ${!selectedBuilding ? 'bg-gray-100 border-gray-300 text-gray-400 cursor-not-allowed' : advancedClicked ? 'bg-holo-coral border-holo-coral text-holo-white' : 'bg-holo-teal/20 border-holo-teal/30 text-holo-teal/70 hover:bg-holo-coral hover:border-holo-coral hover:text-holo-white hover:shadow-lg'} hover:scale-105`}>
              <Sliders size={48} />
      ...
      {showAnalysis && <div className="fixed inset-0 bg-holo-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-holo-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            <div className="p-6">
            ...
            }].map(tab => <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-4 py-2 font-inter font-medium transition-colors duration-200 ${activeTab === tab.id ? 'text-holo-coral border-b-2 border-holo-coral' : 'text-gray-600 hover:text-holo-black'}`}>
                    {tab.label}
                  </button>)}
              </div>
              ...
              <div className="flex justify-end mt-6">
                <button onClick={() => setShowAnalysis(false)} className="px-6 py-2 bg-holo-teal/20 border border-holo-teal text-holo-black rounded-lg font-inter font-medium hover:bg-holo-teal/30 transition-colors duration-200">
                  Close
                </button>
              </div>
            </div>
      ...
      {showAdvanced && <div className="fixed inset-0 bg-holo-black/60 flex items-center justify-center z-50 p-4">
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="number" className="w-full p-3 border border-holo-teal/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-holo-coral" placeholder="Approximate area" />
                </div>
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button onClick={() => setShowAdvanced(false)} className="px-4 py-2 text-gray-600 hover:text-holo-black transition-colors duration-200">
                  Cancel
                </button>
                <button onClick={() => setShowAdvanced(false)} className="px-6 py-2 bg-gradient-teal-coral hover:bg-gradient-coral-teal text-holo-white rounded-lg font-inter font-semibold transition-all duration-300">
                  Apply Changes
                </button>
              </div>
            </div>
